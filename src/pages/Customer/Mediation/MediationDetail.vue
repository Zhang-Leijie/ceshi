  ...
  <div>
    ...
    <div class="detail-main" v-if="info">
      <infor-title title="处理记录">
        <inputText :maxNumber="300" content="请输入处理记录" :answer="mediation" v-model="mediation" slot="title-content">
        </inputText>
      </infor-title>
...
<script>
  ...
  import { getMediation , mediationSave , mediationFinish ,  mediationCancel } from '@/ajax/post.js'
  ...
  export default {
    ...
    methods: {
      save(){
        mediationSave(this.$route.query.id,{
      ...
      cancel() {
        mediationCancel(this.$route.query.id,{
      ...
      finish() {
        mediationFinish(this.$route.query.id,{
          answer:this.mediation,
        }).then((res) => {
          if (res.code == 200) {
            this.$message({
      ...
      getPath() {
        ...
        switch (this.status) {
          case 1:
            path = 'MediationDealing'
            break
          case 2:
            path = 'MediationWaitDeal'
            break
        }
        router.push({
          name: path
