<template>
  <div>
    <div class="detail-header">
      ...
      <el-button size="small" @click="returnList">取消处理</el-button>
      <el-button size="small" @click="dialog.isRecording=true">提交记录</el-button>
    </div>
    <div class="detail-main">
      <infor-title title="用户信息" />
      <infor title="姓名" :con=data.user.name ></infor>
      <infor title="性别" :con=sex ></infor>
      <infor title="手机号" :con=data.user.phone ></infor>
      <infor title="地区" :con=data.user.address ></infor>
      <el-button type="primary" class="detail-main-button">联系用户</el-button>
    </div>
    <div class="detail-main">
      <infor-title title="订单信息"/>
      <infor title="订单号"   :con=data.order.id.toString() />
    ...
    <div class="detail-main" v-if="data.order.status == 4">
      ...
      <el-button type="primary" class="detail-main-button" @click="toView(data.lawyer_info.id,data.order.id)">查看服务</el-button>
    </div>
...
var Url = 'https://api.fa-zai.com/upload/'
 import { orderCancel,orderRecord,orderDetail,orderStopResolve,orderPriceList,orderPhoneSearch,orderAdditional,orderResolve,orderPushOne,orderPushs } from '@/ajax/request.js'
  ...
  export default {
    ...
    methods: {
      ...
      returnList() {
        orderStopResolve(this.data.order.id).then((res)=>{
          if(res.code == 200){
             router.push({
                name: "CommonSelect"
             })
